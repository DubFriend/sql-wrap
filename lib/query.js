...
module.exports = function (_ref) {
  ...
  var squel = _squel2.default.useFlavour(sqlType);
  squel.registerValueHandler(Date, function (d) {
    return d;
  });
  ...
  var runCursor = function runCursor(q, fig) {
    ...
    return self.rows({
      ...
      if (_typeof(resp.results) === 'object' && Array.isArray(resp.results) && typeof resp.resultCount === 'number') {
        ...
        return _bluebird2.default.reject(new TypeError('response is missing "results" and/or "resultCount"'));
      }
    }).then(function (resp) {
      return {
  ...
  self.stream = function (textOrConfig, maybeValues) {
    return driver.stream(resolveRowsConfig(textOrConfig, maybeValues));
  };
    ...
    var wrap = function wrap(method) {
      return function () {
        ...
        s.stream = function () {
          ...
          return self.stream(_lodash2.default.extend({ sql: text, values: values }, fig));
        };

        s.one = function () {
          var fig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

          var _s$toParam3 = s.toParam(),
              text = _s$toParam3.text,
              values = _s$toParam3.values;
        ...
        s.whereIn = function (wheres) {
          if (wheres.length) {
            var ors = squel.expr();
            _lodash2.default.each(wheres, function (where) {
              var ands = squel.expr();
              _lodash2.default.each(where, function (v, k) {
                ands.and((0, _wrapUptick2.default)(k) + ' = ?', v);
              });
              ors.or(ands);
            });
            s.where(ors);
