...
describe('write.unit', function () {
  ...
  describe('update', function () {
    ...
    it('should handle bulk updates', function () {
      return _bluebird2.default.all([(0, _sql.insert)('compoundKey', { a: 'A', b: 'B' }), (0, _sql.insert)('compoundKey', { a: 'M', b: 'N' })]).then(function () {
        return write.update('compoundKey', [{ update: { b: 'C' }, where: { a: 'A' } }, { update: { b: 'O' }, where: { a: 'M' } }]);
      }).then(function () {
        return (0, _sql.all)('compoundKey');
      }).then(function (rows) {
        expect(rows).to.have.same.deep.members([{ a: 'A', b: 'C' }, { a: 'M', b: 'O' }]);
      });
    });

    it('should handle empty array', function () {
      return write.update('compoundKey', []);
    });
  });
